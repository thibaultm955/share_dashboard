...
urls = ['https://finance.yahoo.com/quote/TEM1V.HE?p=TEM1V.HE']
# , 'https://finance.yahoo.com/quote/MLMAD.PA?p=MLMAD.PA', 'https://finance.yahoo.com/quote/LACR.PA?p=LACR.PA'
...
urls.each do |url|
    ...
    if doc.css('h1.D\(ib\)').inner_html == ""
        ...
        hash_info[name]  = {:share_price => share_price, :mnemonic => mnemonic, :variation => variation, :currency => currency, :market => market, :volume => volume, :market_cap => market_cap, :beta => beta, :pe => pe, :eps => eps, :number_of_shares => number_of_shares, :country => country, :sector => sector, :industry => industry, :description => description, :date => date_today, :url => url}
    end
end

hash_info.each do |key, values|
    puts key
    puts values
    # if share already exist, keep it
    if Share.where(:name => key) == []
